function createlogin() {
  ...
  Users.id = "AreaUser";
  Users.type = "text"; // Corriger de "texte" à "text"
  Users.required = true;
  ...
  ErrorDiv.id = "ErrorMessage"; // Identifier l'élément
  ErrorDiv.style.color = "#dc3545"; // Couleur rouge pour l'erreur
  ErrorDiv.style.display = "none"; // Masquer par défaut
  ...
  FormConn.appendChild(ErrorDiv); // Ajouter la div d'erreur
  ...
  Submit.addEventListener("click", async function (event) {
      ...
      try {
          ...
          if (raiponce.ok) {
              ...
              DrawIntra(data);
              ErrorDiv.style.display = "none"; // Cacher le message d'erreur si connexion réussie
          } else {
              // Afficher l'erreur si les identifiants ne sont pas valides
              ErrorDiv.innerText = "le mot de passe ou l'identifiant est invalide"; // Message d'erreur
              ErrorDiv.style.display = "block"; // Afficher le message
          }
      } catch (error) {
          ErrorDiv.innerText = "le mot de passe ou l'identifiant est invalide"; // Afficher le message d'erreur
          ErrorDiv.style.display = "block"; // Afficher le message
      }
  });
}
  ...
  function DrawIntra(data) {
    ...
    fetch("https://zone01normandie.org/api/graphql-engine/v1/graphql", {
      ...
      .then((data) => {
        ...
        upText.setAttribute("fill", "black");
        ...
        downText.setAttribute("fill", "#black");
